...
class Nw(object):
    ...
    def _create_network(self, incoming, num_classes, reuse=None, l2_normalize=True,
        ...
        network = slim.conv2d(
            ...
            padding="SAME", normalizer_fn=self.batch_norm_fn, scope="conv1_1",
        ...
        network = slim.conv2d(
            network, 32, [3, 3], stride=1, activation_fn=nonlinearity,
            padding="SAME", normalizer_fn=self.batch_norm_fn, scope="conv1_2",
            weights_initializer=conv_weight_init, biases_initializer=conv_bias_init,
            weights_regularizer=conv_regularizer)
        if create_summaries:
            tf.summary.histogram(network.name + "/activations", network)
        ...
        network = self.rb.residual_block(
        ...
        network = slim.fully_connected(
            ...
            normalizer_fn=self.batch_norm_fn, weights_regularizer=fc_regularizer,
    ...
    def factory_fn(self, image, reuse, l2_normalize):
